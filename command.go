...
Available Commands:{{range .Commands}}{{if .IsAvailableCommandorHelp}}
...
// IsAvailableCommandorHelp determines if a command is available as non-deprecated
// and non-hidden command (this one does not do the same help check as above fcn)
// Note: hack of IsAvailbleCmd, ie: ends up showing 'help' in usage (not in vendor)
func (c *Command) IsAvailableCommandorHelp() bool {
	if len(c.Deprecated) != 0 || c.Hidden {
		return false
	}
